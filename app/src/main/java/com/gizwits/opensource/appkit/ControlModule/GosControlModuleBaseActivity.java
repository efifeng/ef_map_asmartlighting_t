...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"R"对应seekbar滚动条补偿值
	protected static final int R_OFFSET = 0;
	// 数据点"R"对应数据点增量值
	protected static final int R_ADDITION = 0;
	// 数据点"R"对应数据点定义的分辨率
	protected static final int R_RATIO = 1;
		
	// 数据点"G"对应seekbar滚动条补偿值
	protected static final int G_OFFSET = 0;
	// 数据点"G"对应数据点增量值
	protected static final int G_ADDITION = 0;
	// 数据点"G"对应数据点定义的分辨率
	protected static final int G_RATIO = 1;
		
	// 数据点"B"对应seekbar滚动条补偿值
	protected static final int B_OFFSET = 0;
	// 数据点"B"对应数据点增量值
	protected static final int B_ADDITION = 0;
	// 数据点"B"对应数据点定义的分辨率
	protected static final int B_RATIO = 1;
		
	// 数据点"level"对应seekbar滚动条补偿值
	protected static final int LEVEL_OFFSET = 0;
	// 数据点"level"对应数据点增量值
	protected static final int LEVEL_ADDITION = 0;
	// 数据点"level"对应数据点定义的分辨率
	protected static final int LEVEL_RATIO = 1;
		
	// 数据点"temp"对应seekbar滚动条补偿值
	protected static final int TEMP_OFFSET = 0;
	// 数据点"temp"对应数据点增量值
	protected static final int TEMP_ADDITION = 2700;
	// 数据点"temp"对应数据点定义的分辨率
	protected static final int TEMP_RATIO = 1;
	...
	// 数据点"delete"对应的存储数据
	protected static boolean data_delete;
	// 数据点"R"对应的存储数据
	protected static int data_R;
	// 数据点"G"对应的存储数据
	protected static int data_G;
	// 数据点"B"对应的存储数据
	protected static int data_B;
	// 数据点"level"对应的存储数据
	protected static int data_level;
	// 数据点"temp"对应的存储数据
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				...
				if (dataKey.equals(KEY_DELETE)) {
					data_delete = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_R)) {
			
					data_R = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_G)) {
			
					data_G = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_B)) {
			
					data_B = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LEVEL)) {
			
					data_level = (Integer) map.get(dataKey);
				}
